...
class VAAE(autoencoder):
    ...
    def fit(
        ...
        def create_latent_discriminator():
            ...
            temp_layers.append(
                tf.keras.layers.Dense(units=1, activation='sigmoid', name='latent_discriminator_real_outputs'))
            ...
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(
                tf.keras.layers.Dense(units=1, activation='sigmoid', name='latent_discriminator_fake_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
            ...
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(
                tf.keras.layers.Dense(units=1, activation='sigmoid', name='latent_generator_fake_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
    ...
    def connect_together(self):
        ...
        outputs_dict = {
            ...
            'latent_generator_fake_predition': encoded['latent_generatorr_fake_pred']
        }
