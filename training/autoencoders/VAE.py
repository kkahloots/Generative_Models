...
from graphs.basics.VAE_graph import make_vae
class VAE():
    def __init__(
            self,
            model_name,
    ...
    def fit(self, train_dataset, test_dataset,
        ...
        if random_latent is None:
            random_latent = tf.random.normal(shape=[50, latent_dim])
        generated = self.generate_sample(eps=random_latent)
        ...
        with self.strategy.scope():
            ...
            @tf.function
            def distributed_evaluate_step(dataset_inputs):
                losses_dict = self.compute_losses(feedforward)
                for loss_name, loss_func in losses_dict.items():
                    losses_dict[loss_name] = self.strategy.experimental_run_v2(loss_func, args=(dataset_inputs,))
            ...
            for epoch in epochs_pbar:
                ...
                for i, data_train in enumerate(train_dist_dataset):
                    ...
                    total_loss = distributed_train_step(train_x)
                    tr_losses = distributed_evaluate_step(train_x)
                ...
                for i, data_test in enumerate(test_dist_dataset):
                    ...
                    val_losses = distributed_evaluate_step(test_x)
        ...
        #animate(model_name=self.model_name, path=self.image_gen_dir)
