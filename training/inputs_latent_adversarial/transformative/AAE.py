from graphs.adversarial_graph.AAE_graph import inputs_latent_discriminate_encode_fn
...
class AAE(autoencoder):
    ...
    def fit(
        ...
        def create_inputs_discriminator():
            ...
            temp_layers = tf.keras.models.clone_model(self.get_variables()['generative']).layers
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(tf.keras.layers.Dense(units=1, activation='linear', name='logits_real_discriminator_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
            ...
            temp_layers = tf.keras.models.clone_model(self.get_variables()['generative']).layers
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(tf.keras.layers.Dense(units=1, activation='linear', name='logits_fake_discriminator_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
            ...
            temp_layers = tf.keras.models.clone_model(self.get_variables()['generative']).layers
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(tf.keras.layers.Dense(units=1, activation='linear', name='logits_fake_generator_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
