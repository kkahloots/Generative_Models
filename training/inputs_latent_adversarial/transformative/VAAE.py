...
class VAAE(autoencoder):
    ...
    def inputs_discriminator_real_batch_cast(self,  xt0, xt1):
        ...
        en = autoencoder.encode(self, inputs={'inputs': xt0})
        return {'generative_inputs': en['x_latent'],
                'inputs_discriminator_real_inputs': en['x_latent']
                } ,\
               {
                   'inputs_adversarial_discriminator_real_outputs': self.ONES
               }
    ...
    def inputs_discriminator_fake_batch_cast(self,  xt0, xt1):
        ...
        en = autoencoder.encode(self, inputs={'inputs': xt0})
        return {'generative_inputs': en['x_latent'],
                'inputs_discriminator_fake_inputs': en['x_latent']
                } ,\
               {
                   'inputs_adversarial_discriminator_fake_outputs': self.ZEROS
               }
    ...
    def inputs_generator_fake_batch_cast(self,  xt0, xt1):
        ...
        en = autoencoder.encode(self, inputs={'inputs': xt0})
        return {'generative_inputs': en['x_latent'],
                'inputs_generator_fake_inputs': en['x_latent']
                } ,\
               {
                   'inputs_adversarial_generator_fake_outputs': self.ONES
               }
    ...
    def fit(
        ...
        def create_latent_discriminator():
            ...
            temp_layers.append(
                tf.keras.layers.Dense(units=1, activation='sigmoid', name='latent_adversarial_discriminator_real_outputs'))
            ...
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(
                tf.keras.layers.Dense(units=1, activation='sigmoid', name='latent_adversarial_discriminator_fake_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
            ...
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(
                tf.keras.layers.Dense(units=1, activation='sigmoid', name='latent_adversarial_generator_fake_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
        ...
        def create_inputs_discriminator():
            ...
            temp_layers = tf.keras.models.clone_model(self.get_variables()['generative']).layers
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(tf.keras.layers.Dense(units=1, activation='linear', name='inputs_adversarial_discriminator_real_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
            ...
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(tf.keras.layers.Dense(units=1, activation='linear', name='inputs_adversarial_discriminator_fake_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
            ...
            temp_layers = tf.keras.models.clone_model(self.get_variables()['encoder_mean']).layers
            temp_layers.append(tf.keras.layers.Flatten())
            temp_layers.append(tf.keras.layers.Dense(units=1, activation='linear', name='inputs_adversarial_generator_fake_outputs'))
            temp_layers = tf.keras.Sequential(temp_layers)
    ...
    def connect_together(self):
        ...
        _outputs = {
            ...
            'latent_generator_fake_pred': encoded['latent_generatorr_fake_pred']
        }
        ...
        for i, _output in enumerate(self._AA.output_names):
            if 'tf_op_layer_x_logits' in _output :
                ...
                self._AA.output_names[i] = 'inputs_adversarial_discriminator_fake_outputs'
            elif 'inputs_generator_fake' in _output :
                self._AA.output_names[i] = 'inputs_adversarial_generator_fake_outputs'
            elif 'inputs_discriminator_real' in _output :
                self._AA.output_names[i] = 'inputs_adversarial_discriminator_real_outputs'
    ...
    def latent_discriminator_compile(self, **kwargs):
        ...
        print(self.inputs_latent_generator.summary())

    def inputs_discriminator_compile(self, **kwargs):
        self.inputs_discriminator_real.compile(
            optimizer=self.optimizer,
            loss=self.adversarial_losses['inputs_adversarial_discriminator_real_outputs'](),
            metrics=None
        )
        ...
        self.inputs_discriminator_fake.compile(
            optimizer=self.optimizer,
            loss=self.adversarial_losses['inputs_adversarial_discriminator_fake_outputs'](),
            metrics=None
        )
        ...
        self.inputs_generator_fake.compile(
            optimizer=self.optimizer,
            loss=self.adversarial_losses['inputs_adversarial_generator_fake_outputs'](),
            metrics=None
        )
