...
def infer_prior(latent_mean, regularize=True, lambda_d=100, lambda_od=50):
...
def infer_gaussian_prior(latent_mean, latent_logvariance, regularize=True, lambda_d=100, lambda_od=50):
    cov_enc = tf.linalg.diag(tf.exp(latent_logvariance+epsilon))
