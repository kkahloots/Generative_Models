...
#     shape = img1.shape
#     if len(shape) > 4:
#         conv2d = lambda img: tf.nn.conv2d(img, window, strides=[1,1,1,1], padding='VALID')
#         mu1 = tf.reduce_sum(tf.map_fn(conv2d, img1), axis=1)
#         mu2 = tf.reduce_sum(tf.map_fn(conv2d, img2), axis=1)
#         mu1_sq = mu1*mu1
#         mu2_sq = mu2*mu2
#         mu1_mu2 = mu1*mu2
#
#         sigma1_sq = tf.reduce_sum(tf.map_fn(conv2d, img1*img1), axis=1) - mu1_sq
#         sigma2_sq = tf.reduce_sum(tf.map_fn(conv2d, img2*img2), axis=1) - mu2_sq
#         sigma12 = tf.reduce_sum(tf.map_fn(conv2d, img1*img2), axis=1) - mu1_mu2
#     else:
...
#
#
# def ssim_multiscale(img1, img2, mean_metric=True, level=5):
